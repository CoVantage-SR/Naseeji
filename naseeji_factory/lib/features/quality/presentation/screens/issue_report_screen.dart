...
class _IssueReportScreenState extends ConsumerState<IssueReportScreen> {
  ...
  void _simulateUploadDocument() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('تم إرفاق الملف الداعم بنجاح.')),
    );
  }
  ...
  void _submitIssue() {
    ...
    // Open dispute directs to replacement/return requests screen so the user can choose.
    // Let's go to order details or let them choose to go to return/replacement screen.
    // Automatically open a dispute, and we can offer them to create replacement or return request.
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
                context.push('/orders/${widget.orderId}/replacement-request');
              },
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
                context.push('/orders/${widget.orderId}/return-request');
              },
